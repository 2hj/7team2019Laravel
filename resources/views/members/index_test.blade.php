...
@section('content')
									...
									<div class="disc_container">
										<div>
											<div class="disc_content_2 d-flex flex-column align-items-center justify-content-center">
												<div class="showingDiv" id="showingDiv_{{ $member->id }}">
													<div class="disc_title">{{ $member->name }}</div>
													<div class="disc_subtitle">{{ $member->id }}</div>
    ...
    <script type="text/javascript">
		$(document).ready(function() {
			...
			var showingDivId = null;
			var showHTMLCount = 0;
			function showHTML(id, name, address, mottoes, phone_number) {
				var html = $(`
				<div class="disc_subtitle">${phone_number}</div>
				<div class="disc_subtitle">${address}</div>
				<div class="disc_subtitle">${mottoes}</div>
				`);
				
				showingDivId.append(html);
			}
			...
			$('.showMember').on('click', function(event) {
				...
				console.log('showingDivId:', showingDivId);
				...
				$.ajax({
					...
					success: function(data) {
						...
						showingDivId = $('#showingDiv_'+data['id']);
						console.log('showingDivId:', showingDivId);

						if(showHTMLCount % 2 == 1) {
							showHTML(data['id'], data['name'], data['address'], data['mottoes'], data['phone_number']);
						}
					}
				});
			});
									...
									<div>
										<div class="disc_content_2 d-flex flex-column align-items-center justify-content-center">
											<div>
												...
												<div class="disc_subtitle">${id}</div>
											</div>
										</div>
									</div>
								</div>
